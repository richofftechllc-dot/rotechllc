      ...
      "domains": [
        {
          ...
          "questions": [
            {
              "q": "A crew mass-scans the whole internet for one unpatched service and abandons any host that looks hardened. Which actor is this?",
              "options": [
                "A nation-state group running a patient, months-long intrusion for intelligence",
                "An opportunistic commodity attacker chasing the cheapest possible win at scale",
                "A malicious insider abusing the access they were already granted",
                "A hacktivist crew defacing sites to draw attention to a cause"
              ],
              "answer": 1,
              "exp": "Bo: opportunists don't target YOU — they target a vulnerability and you happened to have it. They optimise for cheap wins at scale, so patching alone kills most of this traffic."
            },
            {
              "q": "The CFO's account moves money at 2am with valid credentials, a normal VPN, and no malware anywhere. What do you suspect first?",
              "options": [
                "A zero-day exploit burned on the VPN appliance's firmware",
                "A distributed denial-of-service attack used as a smokescreen",
                "A supply-chain implant sitting in a signed software update",
                "Stolen credentials or an insider"
              ],
              "answer": 3,
              "exp": "Flo: no exploit needed means none was likely used. Valid credentials behaving abnormally is the signature of credential theft or insider misuse — which is why identity monitoring beats another perimeter box."
            },
            {
              "q": "In the TTP model, how is a 'tactic' best defined?",
              "options": [
                "The attacker's goal at a stage of the operation",
                "The exact tool or command syntax used to carry out a step",
                "The specific procedure one named crew repeats on every job",
                "The vulnerability identifier the attacker chose to exploit"
              ],
              "answer": 0,
              "exp": "Bo: Tactic = the WHY (initial access, privilege escalation, exfiltration). Technique = the HOW. Procedure = the exact way one crew does it. Tools change constantly; tactics barely change at all — learn the sequence, not the tool."
            },
            {
              "q": "Which resource is the free, public matrix of adversary tactics and techniques that the industry uses as a shared language?",
              "options": [
                "The CVE list, which assigns an identifier to each disclosed vulnerability",
                "CVSS, the scoring system that rates how severe a given flaw is",
                "MITRE ATT&CK, a matrix of tactics across the top with techniques beneath each",
                "The OWASP Top 10, which ranks the most common web-application risks"
              ],
              "answer": 2,
              "exp": "Bo: ATT&CK is the shared vocabulary — 'the actor used T1566.001' means a phishing attachment to everyone. CVE names flaws, CVSS scores them, OWASP ranks web risks. Different jobs."
            },
            {
              "q": "In the SolarWinds compromise a signed, legitimate vendor update carried attacker code. What assumption does that most directly break?",
              "options": [
                "That password-complexity requirements meaningfully slow an attacker",
                "That full-disk encryption protects data while a machine is running",
                "That software from a trusted source is safe simply because it is trusted",
                "That email filtering will catch a well-crafted phishing message"
              ],
              "answer": 2,
              "exp": "Flo: the build system itself was compromised, so every downstream check said 'valid.' Trust is not a control — that's the whole argument for assume-breach design and real detection."
            },
            {
              "q": "Colonial Pipeline was reached through a dormant VPN account with no MFA whose password sat in a breach dump. What's the root-cause lesson?",
              "options": [
                "Retire unused accounts and enforce MFA — account hygiene is the actual control",
                "Buy a more comprehensive commercial threat-intelligence subscription",
                "Migrate the affected infrastructure into a modern cloud environment",
                "Increase the frequency and budget of security-awareness training"
              ],
              "answer": 0,
              "exp": "Flo: one forgotten account, one reused password, no MFA — no exotic tradecraft anywhere. The unglamorous controls are the ones that would have mattered."
            },
            {
              "q": "Why does phishing remain the most common way in despite decades of awareness training?",
              "options": [
                "Email protocols are fundamentally impossible to secure in any deployment",
                "The workforce is broadly negligent and ignores the training it receives",
                "Modern email filters provide essentially no protection against it",
                "It is the cheapest reliable path to a goal, and attackers optimise for cost"
              ],
              "answer": 3,
              "exp": "Bo: attackers are economically rational. Nobody burns a valuable zero-day when a convincing email works — which is also why you prioritise by 'cheapest path in,' not 'scariest scenario.'"
            },
            {
              "q": "Your phishing-simulation programme punishes anyone who clicks. What's the predictable second-order effect?",
              "options": [
                "The organisation's overall click rate falls and genuine risk drops with it",
                "People stop reporting real phishing, killing the control that shortens incidents",
                "Attackers abandon email and switch primarily to voice-based social engineering",
                "There is no measurable behavioural effect from the punishment either way"
              ],
              "answer": 1,
              "exp": "Flo: punishment kills reporting, and a fast report is what shortens dwell time. Measure report rate, not just click rate — run simulations to fix the process, never to blame the person."
            }
          ]
        },
        {
          ...
          "questions": [
            {
              "q": "A critical-severity flaw sits on an isolated lab box with no network path; a medium sits on the internet-facing app holding customer records. Which is the higher RISK?",
              "options": [
                "The critical flaw, because severity ratings are what decide priority",
                "The medium on the internet-facing app, because risk weighs likelihood and impact",
                "They are equal, since both represent vulnerabilities that must be patched",
                "Neither can be judged until a penetration test confirms exploitability"
              ],
              "answer": 1,
              "exp": "Bo: Risk = likelihood x impact. Severity describes the flaw in isolation; risk describes what it means to YOUR business. Confusing the two is the most common junior mistake."
            },
            {
              "q": "Which assessment answers 'what could go wrong with this design?' before any code is written?",
              "options": [
                "A penetration test, which chains real weaknesses to prove impact",
                "A vulnerability scan, which finds known flaws across the estate",
                "A compliance audit, which checks controls against a requirement set",
                "Threat modelling, done on a whiteboard at design time before the system exists"
              ],
              "answer": 3,
              "exp": "Flo: threat modelling is the cheapest control there is and the most skipped. Everything else in the list needs a built system to test — threat modelling happens before there's anything to break."
            },
            {
              "q": "Your organisation passed its compliance audit with zero findings. What can you conclude about its security?",
              "options": [
                "Only that it met the assessed controls, in scope, at that point in time",
                "That it is secure, because a clean audit reflects a strong posture",
                "That an upcoming penetration test will also come back clean",
                "That its risk has now been effectively transferred to the auditor"
              ],
              "answer": 0,
              "exp": "Flo: compliance is a floor, not a ceiling, and it's scoped and point-in-time. Plenty of breached companies were fully compliant the week before."
            },
            {
              "q": "A scanner returns 4,000 findings. What is the actual skill you're paid for?",
              "options": [
                "Patching every one of the 4,000 findings within a 30-day window",
                "Rescanning weekly so leadership can watch the trend line move",
                "Turning the list into a prioritised plan tied to real business impact",
                "Forwarding the full report to each system owner for action"
              ],
              "answer": 2,
              "exp": "Bo: the scan is the easy part — anyone can run it. 'These six, this quarter, in this order, and here's why' is the judgement that gets you hired and promoted."
            },
            {
              "q": "Leadership decides not to fix a known risk this year. What makes that a legitimate decision rather than negligence?",
              "options": [
                "The risk having been rated medium or lower on the severity scale",
                "A compensating security tool already being present in the environment",
                "Documented acceptance with a named owner and a scheduled review date",
                "A verbal sign-off given by a manager during a status meeting"
              ],
              "answer": 2,
              "exp": "Flo: Accept is valid alongside Mitigate, Transfer and Avoid — but only when it's written down, owned by a named person, and revisited. Undocumented acceptance is just an unmanaged risk."
            },
            {
              "q": "Which best describes your attack surface?",
              "options": [
                "Every point an attacker could interact with, external and internal alike",
                "The current count of unpatched CVEs across the whole estate",
                "The public IP ranges the organisation has registered and exposed",
                "The systems formally listed in scope for the annual penetration test"
              ],
              "answer": 0,
              "exp": "Bo: public IPs, APIs, SaaS tenants, VPN endpoints, staff inboxes, third parties with a login. Most organisations can't list theirs — which is why inventory is where every framework starts."
            },
            {
              "q": "Why is asset inventory the foundation for the rest of a security programme?",
              "options": [
                "Auditors require a formal asset register as a compliance artifact",
                "It provides the basis for allocating software-licence costs fairly",
                "Cyber-insurance underwriters demand one before issuing a policy",
                "You cannot protect, monitor or patch what you don't know you own"
              ],
              "answer": 3,
              "exp": "Bo: coverage gaps are exactly where incidents live. 'We have EDR' means nothing until you can say what percentage of KNOWN assets it's on — and how good that known list actually is."
            },
            {
              "q": "A penetration test comes back clean. What's the most accurate reading?",
              "options": [
                "The environment is secure and the finding can be reported upward",
                "No exploitable path was found in that scope during that time window",
                "The vulnerability-management programme can safely be paused",
                "Controls are demonstrably operating effectively across the estate"
              ],
              "answer": 1,
              "exp": "Flo: pentests are narrow, point-in-time and scoped. Clean means clean that week, in that scope, by that tester — treating it as a certificate of health is how organisations get surprised."
            }
          ]
        },
        {
          ...
          "questions": [
            {
              ...
              "options": [
                "The sheer volume of indicators it delivers each day",
                "Whether it changes a decision or an action you actually take",
                "The number of distinct threat actors it claims to track",
                "How quickly it delivers new indicators in real time"
              ],
              "answer": 1,
              "exp": "Bo: if nothing is done differently because of it, you're buying anxiety, not intelligence. Intel earns its cost by changing what you hunt, patch, block or fund."
            },
            {
              ...
              "options": [
                "Tactical intelligence, meant to be consumed directly by machines",
                "Operational intelligence, telling defenders what to hunt this week",
                "Technical intelligence, describing a single exploit's inner workings",
                "Strategic intelligence, aimed at leadership, budget and long-term priorities"
              ],
              "answer": 3,
              "exp": "Flo: strategic intel informs leadership and budget. Operational tells defenders what to hunt now; tactical is machine-consumable indicators for blocklists. Know which one you're holding."
            },
            {
              "q": "Which type of intelligence has the SHORTEST useful shelf life?",
              "options": [
                "Atomic indicators such as IP addresses and file hashes",
                "Strategic trends describing how a whole sector is being targeted",
                "The tactics, techniques and procedures a group habitually uses",
                "The underlying motivations and objectives that drive an actor"
              ],
              "answer": 0,
              "exp": "Bo: an attacker changes an IP or recompiles for a new hash in seconds. Behaviour is far more durable — detections built on TTPs outlive detections built on indicators."
            },
            {
              "q": "When prioritising defensive work, what's the most useful question to ask about an attacker?",
              "options": [
                "What is the most damaging thing they could theoretically ever do?",
                "Which unpublished zero-day exploits might they be holding in reserve?",
                "What is their cheapest path to the goal from where they currently stand?",
                "How large is the operating budget behind the group targeting us?"
              ],
              "answer": 2,
              "exp": "Bo: attackers optimise for cost. Defending the scariest imaginable scenario while a 2019 CVE on the VPN stays open is how organisations spend a lot and stay exposed."
            },
            {
              "q": "You want to practise offensive techniques to become a better defender. What's non-negotiable?",
              "options": [
                "Holding a recognised offensive-security certification beforehand",
                "Routing all of your practice traffic through an anonymising VPN",
                "Written authorisation and a clearly defined scope for anything you test",
                "Getting informal approval from your direct manager and no one else"
              ],
              "answer": 2,
              "exp": "Flo: testing systems you don't own or aren't contracted to test is a crime in most places — skill and intent are irrelevant. Build the habit in your own lab, where the only thing at stake is your machine."
            },
            {
              "q": "Which best describes responsible disclosure?",
              "options": [
                "Reporting privately to the owner with a reasonable window to fix before going public",
                "Publishing working exploit code immediately to maximise public awareness",
                "Selling the finding discreetly to whichever buyer offers the most money",
                "Keeping the finding secret indefinitely under any and all circumstances"
              ],
              "answer": 0,
              "exp": "Flo: private report, agreed remediation window, then coordinated publication. It gets defenders a fix while still letting the finding inform the wider industry."
            },
            {
              "q": "Your team subscribes to five feeds and nobody has opened the dashboard in three months. What's the correct first move?",
              "options": [
                "Purchase a threat-intelligence platform to aggregate the five feeds",
                "Assign a junior analyst to read every feed thoroughly each day",
                "Raise the alerting threshold so only the loudest items surface",
                "Cut to the feeds that drive a defined action and wire those into a workflow"
              ],
              "answer": 3,
              "exp": "Bo: aggregating unread feeds just produces a more expensive unread feed. Start from the decision you want to make, then keep only the input that informs it."
            },
            {
              "q": "Why do detections built on attacker BEHAVIOUR outperform detections built on indicators?",
              "options": [
                "Behaviour-based detections generate far fewer alerts across the board",
                "Behaviour is expensive for an attacker to change; indicators are trivial to change",
                "Behaviour-based detections require essentially no ongoing tuning effort",
                "Indicator-based detections cannot be automated in a modern SIEM"
              ],
              "answer": 1,
              "exp": "Flo: changing an IP or hash is free; changing HOW you achieve privilege escalation means rebuilding tradecraft. Defend at the layer that costs the attacker the most."
            }
          ]
        },
        {
          ...
          "questions": [
            {
              "q": "Which of these is a genuine outcome metric rather than an activity metric?",
              "options": [
                "The number of attacks the perimeter blocked over the quarter",
                "Mean time to detect, which measures how fast you notice an intrusion",
                "The count of vulnerability scans the team ran in the period",
                "The percentage of staff who completed the annual training"
              ],
              "answer": 1,
              "exp": "Bo: blocked traffic is weather — it measures the internet, not you. MTTD measures capability. If a metric rises when you do more work but stays flat when you get safer, it's vanity."
            },
            {
              "q": "Security posture is best described as the combination of which three things?",
              "options": [
                "Firewalls, antivirus and encryption working together in layers",
                "The compliance certifications the organisation currently holds",
                "The total annual budget allocated to the security function",
                "Technology that's deployed, the process behind it"
              ],
              "answer": 3,
              "exp": "Flo: technology you've deployed AND configured AND monitor; the process for what happens at 2am; and whether people report the weird email. The third is the most underfunded."
            },
            {
              "q": "Your EDR is deployed on 62% of known assets. Why is that metric more useful than 'we have EDR'?",
              "options": [
                "Coverage gaps are precisely where incidents take hold and spread",
                "It provides a clean justification for renewing the EDR licence",
                "It lets the team benchmark the tool against industry peers",
                "It gives the auditor a concrete number to record as evidence"
              ],
              "answer": 0,
              "exp": "Bo: a control that isn't everywhere isn't a control, it's a sample. And the harder follow-up is how complete the 'known assets' list it's measured against actually is."
            },
            {
              "q": "Which pair of metrics best captures a detection-and-response capability?",
              "options": [
                "Total alert volume and the number of detection rules in place",
                "Training-completion rate and the phishing-simulation click rate",
                "Mean time to detect and mean time to respond, taken together",
                "Patches applied per month and the frequency of vulnerability scans"
              ],
              "answer": 2,
              "exp": "Bo: time to detect and time to respond. Detection without response is just knowing you're on fire. Alert volume tells you about tuning, not effectiveness."
            },
            {
              "q": "In awareness programmes, which measure best reflects real behaviour change?",
              "options": [
                "The training-completion percentage across the whole workforce",
                "The average quiz score staff achieve on the awareness module",
                "The phishing report rate — whether people actually raise their hand",
                "The total number of training hours delivered over the year"
              ],
              "answer": 2,
              "exp": "Flo: completion measures attendance; reporting measures whether someone will raise their hand when it counts — and a fast report is what shortens an incident."
            },
            {
              "q": "A maturity assessment scores you 2 in asset management and 4 in incident response. What's the right response?",
              "options": [
                "Invest against the weak area rather than chasing a uniform score",
                "Re-score the assessment to present a single consistent level",
                "Set a target of reaching level 5 across every category",
                "Discard maturity models on the grounds that they're subjective"
              ],
              "answer": 0,
              "exp": "Flo: every organisation is uneven. The value is honesty about where you're thin, so spend follows risk. Chasing a flat top score is how programmes waste money looking tidy."
            },
            {
              "q": "Which of these is LEAST useful to report to a board?",
              "options": [
                "The trend in mean time to respond over the last four quarters",
                "The percentage of critical vulnerabilities remediated within SLA",
                "Control coverage across the organisation's crown-jewel systems",
                "The total volume of firewall events blocked during the period"
              ],
              "answer": 3,
              "exp": "Bo: event volume is unactionable at board level and invites the wrong question. Boards need risk posture and trend, in terms they can make funding decisions against."
            },
            {
              "q": "Why measure 'percentage of critical vulnerabilities remediated within SLA' rather than 'vulnerabilities found'?",
              "options": [
                "Finding fewer issues over time is itself a sign of better security",
                "Closure is the outcome that matters; discovery is only the input",
                "It meaningfully reduces the organisation's scanner-licensing cost",
                "Auditors specifically require remediation rates over discovery counts"
              ],
              "answer": 1,
              "exp": "Bo: finding them isn't the job — closing them is. A programme that finds thousands and fixes dozens is a reporting exercise, not a security function."
            }
          ]
        },
        {
          ...
          "questions": [
            {
              "q": "A credible security strategy answers four questions. Which set?",
              "options": [
                "Which vendors, at what cost, on what timeline, approved by whom",
                "What we protect, from whom, to what standard, and what we do first",
                "How many staff, which tools, which certifications, which SLAs",
                "Which regulations, which auditors, what evidence, by which dates"
              ],
              "answer": 1,
              "exp": "Bo: assets, adversaries, standard, sequence. A document that doesn't answer those four is a wish list with a budget attached."
            },
            {
              "q": "Which framework is the strongest starting point when you have essentially nothing and need to know what to do FIRST?",
              "options": [
                "ISO 27001, because it certifies a full management system",
                "NIST CSF, because it's the best shared language with leadership",
                "COBIT, because it maps security to broader IT governance",
                "The CIS Controls, because they're prioritised and tell you the order"
              ],
              "answer": 3,
              "exp": "Flo: CIS Controls are prioritised and opinionated — they tell you the order. NIST CSF is the better shared language with leadership; ISO 27001 is the better answer when customers want certified proof."
            },
            {
              "q": "Zero trust is best described as which of the following?",
              "options": [
                "An operating principle: verify explicitly, least privilege, assume breach",
                "A product you deploy at the network perimeter to inspect traffic",
                "A programme to remove every VPN from the environment entirely",
                "A regulatory requirement that certain sectors must comply with"
              ],
              "answer": 0,
              "exp": "Bo: no vendor sells you zero trust. It's a design stance — stop trusting things for where they sit on the network, verify every request, and assume something is already compromised."
            },
            {
              "q": "Which funding request is most likely to be approved?",
              "options": [
                "'We need $80,000 for a privileged-access solution to follow best practice'",
                "'Our main competitors already run privileged-access management and we don't'",
                "'Fourteen accounts can change production unreviewed",
                "'The auditors flagged privileged access as a gap in this year's report'"
              ],
              "answer": 2,
              "exp": "Bo: denominate it in THEIR risk, not your tooling. Same money, completely different conversation — the winning one names the exposure, the consequence, and the business unlock."
            },
            {
              "q": "Before buying a new security product, what's the highest-value question?",
              "options": [
                "Which vendor currently leads the relevant analyst market quadrant?",
                "What is the full three-year total cost of ownership of the tool?",
                "What are we already paying for that's unconfigured, uncovered or unmonitored?",
                "Will the product integrate cleanly with our existing SIEM platform?"
              ],
              "answer": 2,
              "exp": "Bo: 'we already own it and it's turned off' is the most common finding in the industry. Fixing that on a budget of zero is how a new hire makes their name in 90 days."
            },
            {
              "q": "You're running a proof-of-concept for a security tool. What most determines whether it's meaningful?",
              "options": [
                "Your environment, your data, and success criteria written down beforehand",
                "The vendor-supplied test data running in their demo environment",
                "The length of the trial period the vendor is willing to grant",
                "The number of stakeholders who attend the vendor's live demo"
              ],
              "answer": 0,
              "exp": "Flo: every product demos beautifully on the vendor's laptop. Written criteria agreed in advance are what stop a POC becoming a sales meeting with extra steps."
            },
            {
              "q": "Three deployed products overlap heavily in capability. What often buys more security than a fourth purchase?",
              "options": [
                "Adding the fourth product for an extra layer of defence in depth",
                "Rotating to a different vendor for that capability every year",
                "Splitting the capability's coverage across separate business units",
                "Consolidating, then fully configuring and monitoring what remains"
              ],
              "answer": 3,
              "exp": "Bo: overlap costs money and attention while creating gaps between consoles nobody owns. Depth means layered CONTROLS, not duplicate products sitting in monitor-only mode."
            },
            {
              "q": "For a new security hire, why ship a small visible fix inside the first 90 days?",
              "options": [
                "It formally satisfies the probationary requirement for the role",
                "Credibility compounds",
                "It demonstrates hands-on proficiency with the security tooling",
                "It measurably reduces the outstanding security-assessment backlog"
              ],
              "answer": 1,
              "exp": "Bo: learn, assess, then act. Something visible early buys the trust that gets the expensive, slower work approved."
            }
          ]
        }
      ]
      ...
      "domains": [
        {
          ...
          "questions": [
            {
              "q": "You spot a weak default password on a company's public login page. You don't work there and have no contract. What's the correct action?",
              "options": [
                "Log in once to confirm the flaw is real, then report what you found",
                "Report it responsibly without ever accessing the system yourself",
                "Test it quietly since you're ultimately doing the company a favour",
                "Use it a single time to prove impact, then immediately stop there"
              ],
              "answer": 1,
              "exp": "Flo: any access without authorisation is a crime, even to 'prove' a real flaw and even with good intent. Responsible disclosure means reporting WITHOUT touching it — the line is authorisation, not intention."
            },
            {
              "q": "What single factor separates a penetration tester from a criminal running the exact same commands?",
              "options": [
                "The specific set of tools and frameworks each one chooses to use",
                "Whether the person holds a recognised offensive-security certification",
                "Whether the activity ends up causing any actual damage to systems",
                "Written authorisation and a defined scope for the work being done"
              ],
              "answer": 3,
              "exp": "Bo: identical commands, identical tools. One has a signed scope document, the other has a case number. Authorisation is the whole difference."
            },
            {
              "q": "Before a real engagement starts, which document defines what you can test, when, and who to call if something breaks?",
              "options": [
                "The rules of engagement, agreed and signed before any testing begins",
                "The final vulnerability report delivered at the end of the job",
                "The master service agreement covering the overall commercial terms",
                "The published CVE list for the software in the target environment"
              ],
              "answer": 0,
              "exp": "Flo: the RoE is step zero — scope, timing, off-limits systems, emergency contacts, and whether you may exploit or only demonstrate. No signed RoE, no testing."
            },
            {
              "q": "Which best captures how attackers actually approach getting in?",
              "options": [
                "They seek out and deploy the most sophisticated exploit available",
                "They almost always begin an intrusion by burning a fresh zero-day",
                "They take the cheapest path of least resistance to the goal",
                "They deliberately attack the most heavily defended system first"
              ],
              "answer": 2,
              "exp": "Bo: attackers are economically rational — a valid password beats a zero-day. Defenders and testers should check the easy doors before reaching for the exotic ones."
            },
            {
              "q": "Experienced testers say enumeration is 'the whole game.' What does that mean in practice?",
              "options": [
                "Running as many automated scans against the target as possible",
                "Exploiting every open port the moment a scan reveals it",
                "Reading carefully what you find",
                "Automating the entire assessment from recon through to reporting"
              ],
              "answer": 2,
              "exp": "Bo: the skill is looking, not exploiting. The misconfigured service with the default password was visible the whole time — the pro is the one who actually read it."
            },
            {
              "q": "Why should offensive-security practice happen on an isolated, host-only lab network?",
              "options": [
                "It guarantees your practice can never reach a system you don't own",
                "It runs noticeably faster than a routed or bridged network would",
                "It is a formal prerequisite for most offensive-security certifications",
                "It hides your practice traffic from your internet service provider"
              ],
              "answer": 0,
              "exp": "Flo: host-only networking is the technical guarantee that a mistake in your lab stays in your lab. It's how you keep 'practice' from becoming 'unauthorised access to someone else's network.'"
            },
            {
              ...
              "options": [
                "Download a small representative sample as concrete proof of impact",
                "Exfiltrate the full dataset to show the true worst-case scenario",
                "Take the data now and ask the client for forgiveness afterwards",
                "Prove you could read it without copying anything out, and document that"
              ],
              "answer": 3,
              "exp": "Flo: proving impact WITHOUT causing it is a core professional skill and exactly what the RoE demanded. Exceeding scope — even to be thorough — breaks the engagement and possibly the law."
            },
            {
              "q": "What is the standard methodology loop a tester or attacker follows?",
              "options": [
                "Exploit the first weakness, escalate to admin, then exit quietly",
                "Recon, then scan and enumerate, gain access, escalate and move",
                "Scan the target, report the open ports, and hand off for patching",
                "Phish a user, dump their credentials, and sell them onward"
              ],
              "answer": 1,
              "exp": "Bo: the same repeatable sequence every time, CTF box or enterprise engagement. Learning the loop means no target is ever a blank page."
            }
          ]
        },
        {
          ...
          "questions": [
            {
              "q": "What distinguishes passive from active reconnaissance?",
              "options": [
                "Passive recon is lawful whereas active recon is inherently illegal",
                "Passive never touches the target; active interacts with it and leaves traces",
                "Passive recon is tool-driven while active recon is always manual",
                "There is no meaningful operational difference between the two"
              ],
              "answer": 1,
              "exp": "Bo: passive recon reads what's already public — you're invisible to the target. Active recon interacts directly, so it can be logged. Passive first, always."
            },
            {
              "q": "A company's job posting lists the exact software and versions its team supports. To an attacker, that is:",
              "options": [
                "Irrelevant marketing material with no security significance",
                "A legal safe harbour that authorises probing those systems",
                "Evidence that the company maintains a mature security posture",
                "Free reconnaissance revealing the organisation's technology stack"
              ],
              "answer": 3,
              "exp": "Bo: OSINT gold. Job posts, employee profiles and public docs hand attackers the tech stack, the org chart, and who to phish — all without touching the target."
            },
            {
              "q": "Why do experienced testers call a service VERSION NUMBER the pivot point of an engagement?",
              "options": [
                "You match it against known-vulnerability databases to find a way in",
                "It confirms that the port scan completed without any errors",
                "It identifies the vendor so the final report can credit them",
                "It reveals which network port the service is listening on"
              ],
              "answer": 0,
              "exp": "Bo: 'service X version 2.1.3' → search CVE and exploit archives → is there a known issue for exactly that version? That loop turns a scan result into an entry point."
            },
            {
              "q": "You scan a target and find three open ports. A beginner fires exploits immediately. What does a professional do first?",
              "options": [
                "Report the three open ports to the client and await instructions",
                "Close the ports to reduce the target's exposed attack surface",
                "Deeply enumerate the services behind those ports before touching anything",
                "Rescan the target from a different source IP to avoid detection"
              ],
              "answer": 2,
              "exp": "Bo: the open port isn't the finding — the misconfigured service behind it is. Enumeration before exploitation separates results from noise."
            },
            {
              "q": "Which activity is genuinely PASSIVE reconnaissance?",
              "options": [
                "Port-scanning the target's public-facing servers for open ports",
                "Connecting to the target's SSH service to grab its version banner",
                "Reading certificate-transparency logs and breach-database exposure",
                "Pinging the target's address range to see which hosts respond"
              ],
              "answer": 2,
              "exp": "Bo: certificate-transparency logs and breach data are public — you never touch the target to read them. Port scans, banner grabs and pings all interact with it directly."
            },
            {
              "q": "What's the defensive discipline that mirrors an attacker's recon — running recon on your OWN organisation to see what leaks?",
              "options": [
                "Attack surface management, run regularly against your own footprint",
                "Penetration testing, commissioned once or twice a year externally",
                "Incident response, activated only after a breach has been detected",
                "Threat hunting, focused on adversaries already inside the network"
              ],
              "answer": 0,
              "exp": "Flo: the best blue teams recon themselves regularly. Anything an attacker can find about you in passive recon is something you can find first and lock down."
            },
            {
              "q": "Enumeration reveals a file share requiring no authentication. What's actually been found?",
              "options": [
                "An open port with no further security significance on its own",
                "A false positive that a second scan will almost certainly clear",
                "A honeypot deliberately placed to detect and study intruders",
                "A misconfiguration that may expose data or provide a foothold"
              ],
              "answer": 3,
              "exp": "Bo: an unauthenticated share is a classic misconfiguration — potential data exposure and a possible way in. Exactly the kind of thing enumeration surfaces that a shallow scan misses."
            },
            {
              "q": "Why is OSINT on employees a serious risk even though it never touches company systems?",
              "options": [
                "It technically violates computer-misuse law the moment it's collected",
                "It hands attackers the org chart, reused usernames and phishing targets",
                "It reliably triggers the target's intrusion-detection systems",
                "It only produces useful results against very small companies"
              ],
              "answer": 1,
              "exp": "Bo: people are attack surface. A reused username across sites, a role in a profile, an email in a breach dump — none of it touches a server, and all of it fuels the next step."
            }
          ]
        },
        {
          ...
          "questions": [
            {
              ...
              "options": [
                "Through novel zero-day exploits developed for the specific target",
                "Through boring doors: weak credentials, unpatched flaws, misconfigs",
                "Through physical break-ins to the organisation's facilities",
                "Through implants hidden in the hardware supply chain"
              ],
              "answer": 1,
              "exp": "Bo: zero-days are rare and expensive. The overwhelming majority of real intrusions start with a credential, an unpatched CVE, a misconfig, or a phish. The boring doors are the doors."
            },
            {
              "q": "What is privilege escalation?",
              "options": [
                "Moving from one compromised machine across to the next machine",
                "Stealing a valid password from a user during a phishing attack",
                "Copying sensitive data out of the environment to an external host",
                "Going from a low-privilege user to admin or root on the current machine"
              ],
              "answer": 3,
              "exp": "Bo: escalation is getting bigger on the box you already have — usually via a misconfiguration, a loose file permission, a hijackable scheduled task, or an unpatched local flaw."
            },
            {
              "q": "What is lateral movement?",
              "options": [
                "Using a machine you control to reach the next one",
                "Elevating your privileges to root on the host you already own",
                "Scanning an internal subnet to enumerate the hosts on it",
                "Deleting log entries to hide the traces of an intrusion"
              ],
              "answer": 0,
              "exp": "Bo: lateral movement is spreading. You landed on one box; the target is elsewhere. Credentials found on the first machine are the usual vehicle — which is why credential hygiene is everything."
            },
            {
              "q": "Why is a flat network with no segmentation called the attacker's best friend?",
              "options": [
                "It responds to scans faster than a properly segmented network",
                "It typically has no firewalls deployed anywhere in the environment",
                "Compromising one machine gives reach to every other machine",
                "It usually relies on weak encryption between its internal hosts"
              ],
              "answer": 2,
              "exp": "Bo: if one box reaches all boxes, the game ends at step one. That's the NotPetya lesson in a sentence — and why segmentation is the control defenders lose sleep over."
            },
            {
              "q": "On an authorised engagement you achieve persistence and could exfiltrate data. What's the professional default?",
              "options": [
                "Exfiltrate a full copy to demonstrate the genuine worst case",
                "Establish permanent access so the client can be re-tested later",
                "Prove you could, document it, and don't take data unless scope allows",
                "Leave a discreet backdoor behind for the client's convenience"
              ],
              "answer": 2,
              "exp": "Flo: demonstrating impact without causing it is the mature deliverable. Actually exfiltrating or leaving backdoors exceeds normal scope and creates real risk."
            },
            {
              "q": "A service runs as root and a low-privilege user can influence its configuration. This is a setup for what?",
              "options": [
                "Privilege escalation, leveraging the service's high privilege to elevate yours",
                "Lateral movement across to a separate machine on the network",
                "Data exfiltration of files out to an attacker-controlled server",
                "Passive reconnaissance of the target before any interaction"
              ],
              "answer": 0,
              "exp": "Bo: a root-level service you can influence is a classic local privilege-escalation path — you use the service's high privilege to elevate your own on that machine."
            },
            {
              "q": "Which control most directly limits the damage of lateral movement after one machine is compromised?",
              "options": [
                "Antivirus software installed on every endpoint in the environment",
                "A stronger firewall ruleset at the network's outer perimeter",
                "Enforcing longer minimum password lengths for all user accounts",
                "Network segmentation combined with least privilege on accounts"
              ],
              "answer": 3,
              "exp": "Flo: once the attacker is inside, the perimeter is behind them. Segmentation contains where they can go, and least privilege limits what stolen credentials can do."
            },
            {
              "q": "Why do defenders study the attacker's post-access playbook of escalation, movement and persistence?",
              "options": [
                "So they can eventually transition into offensive red-team roles",
                "So they can detect and contain an intrusion already in progress",
                "Because compliance frameworks specifically mandate the study of it",
                "So they can write more restrictive outbound firewall rules"
              ],
              "answer": 1,
              "exp": "Flo: prevention fails sometimes. Knowing what an attacker does AFTER getting in is what lets a blue team spot the escalation or the lateral move and cut it off before it's a breach."
            }
          ]
        },
        {
          ...
          "questions": [
            {
              ...
              "options": [
                "They never expire and so can be reused indefinitely over time",
                "They let the attacker act like a legitimate user, which is hard to detect",
                "They function identically across every system in the environment",
                "They command a consistently higher resale price on criminal markets"
              ],
              "answer": 1,
              "exp": "Bo: a stolen credential walks the front door looking exactly like a real user. That legitimacy is precisely what makes credential-based attacks so effective and so hard to catch."
            },
            {
              "q": "An attacker with admin on a machine reads cached credentials from its memory. Which single control most reduces this risk?",
              "options": [
                "Enforcing significantly longer minimum password lengths",
                "Deploying a faster antivirus engine on the affected host",
                "Disabling the machine's network interface when not in use",
                "Limiting who holds admin or local-admin rights — least privilege"
              ],
              "answer": 3,
              "exp": "Flo: credential dumping requires admin on the box. Least privilege shrinks how many machines an attacker can do this on — and detecting the dumping tools covers the rest."
            },
            {
              "q": "'Credential stuffing' succeeds because of which user behaviour?",
              "options": [
                "Reusing the same password across many different sites and services",
                "Choosing passwords that fail the site's complexity requirements",
                "Writing passwords down on paper near the workstation",
                "Sharing a single account login among several team members"
              ],
              "answer": 0,
              "exp": "Bo: one password breached elsewhere, tried everywhere. Unique passwords and a manager break it; MFA makes a stolen password insufficient on its own."
            },
            {
              "q": "What is the single most effective control against stolen or dumped passwords?",
              "options": [
                "Forcing a password rotation for every user every thirty days",
                "Increasing the minimum required password length substantially",
                "Multi-factor authentication enforced everywhere it possibly can be",
                "Locking an account automatically after three failed attempts"
              ],
              "answer": 2,
              "exp": "Bo: MFA turns a stolen password into just a string — it's not enough by itself. If you learn one defensive lesson from studying credential attacks, it's this."
            },
            {
              "q": "A developer commits an API key to a public code repository. This is an example of what?",
              "options": [
                "Credential dumping from the memory of a compromised host",
                "Credential stuffing using a password reused across sites",
                "Exposed secrets — credentials leaked out into the open ground",
                "A man-in-the-middle interception of traffic on the network"
              ],
              "answer": 2,
              "exp": "Bo: secrets in code, config files or shared docs are credentials lying on the ground. The defence is automated secret scanning in the pipeline plus fast key rotation when one leaks."
            },
            {
              "q": "Why does capturing credentials 'in transit' succeed on some networks?",
              "options": [
                "The traffic isn't properly encrypted or the network isn't segmented",
                "The passwords in use are simply too short to resist capture",
                "The attacker already holds administrative rights on the host",
                "Multi-factor authentication has been enabled for the accounts"
              ],
              "answer": 0,
              "exp": "Bo: interception exploits unencrypted or poorly segmented networks. Encryption in transit and segmentation close it — it's a network-hygiene problem, not a password-strength one."
            },
            {
              "q": "You're a defender who just learned how credential dumping works. What's the most valuable thing that knowledge gives you?",
              "options": [
                "The ability to perform credential dumping yourself on demand",
                "A recognised credential you can add to your professional resume",
                "A concrete justification to buy several additional security tools",
                "The ability to prioritise the exact controls and detections that break it"
              ],
              "answer": 3,
              "exp": "Flo: understanding the attack is what lets you rank the defence — least privilege, MFA, and detection of the specific tools. Red knowledge directly sharpens blue prioritisation."
            },
            {
              "q": "Which combination best defends the credential attack surface overall?",
              "options": [
                "Antivirus software paired with a well-configured network firewall",
                "MFA, least privilege, credential monitoring",
                "Longer passwords combined with a mandatory thirty-day rotation",
                "A single sign-on portal fronting all of the internal applications"
              ],
              "answer": 1,
              "exp": "Bo: credentials get taken many ways, so the defence is layered — each control breaks a different theft path. No single setting covers dumping, reuse, interception and leaked secrets at once."
            }
          ]
        },
        {
          ...
          "questions": [
            {
              "q": "Why does this track teach tool CATEGORIES rather than specific tools?",
              "options": [
                "Naming specific offensive tools would be legally prohibited to do",
                "Tools change every few years, but the category — the job it does",
                "Categories happen to be easier to write exam questions about",
                "It is done mainly to avoid appearing to endorse any one vendor"
              ],
              "answer": 1,
              "exp": "Bo: understand 'a thing that intercepts web traffic' and you can pick up any web proxy in an afternoon. Memorise one tool by rote and you're stuck the moment the shop uses a different one."
            },
            {
              "q": "What's the job of a vulnerability scanner, as distinct from a port scanner?",
              "options": [
                "Cracking captured password hashes against a large wordlist",
                "Intercepting and modifying a web application's HTTP traffic",
                "Organising exploits and payloads for use during an engagement",
                "Matching discovered software against known-vulnerability databases at scale"
              ],
              "answer": 3,
              "exp": "Bo: a port scanner finds open doors and versions. A vulnerability scanner takes it further — cross-referencing versions against known CVEs across many hosts quickly."
            },
            {
              "q": "On a penetration-testing engagement, what is the actual PRODUCT the client pays for?",
              "options": [
                "The report — what's wrong, how bad it is, and exactly how to fix it",
                "Proof, with evidence, that you successfully got into the systems",
                "A complete list of the exploits and tools used during testing",
                "Working credentials and access to the systems that were tested"
              ],
              "answer": 0,
              "exp": "Flo: getting in is how you learn what to write. The report is the deliverable — a brilliant hack with a lazy write-up is a failed engagement."
            },
            {
              "q": "Which is the strongest way to express IMPACT in a report finding?",
              "options": [
                "'This issue carries a CVSS base score of 8.1 (high)'",
                "'The affected network port was found to be open'",
                "'An unauthenticated attacker could read every customer record'",
                "'The exploit succeeded on the very first attempt'"
              ],
              "answer": 2,
              "exp": "Flo: impact in business terms lands with the people who fund the fix. A raw score means nothing to a decision-maker; 'an attacker could read every customer record' means everything."
            },
            {
              "q": "A good report finding has five parts. Which set?",
              "options": [
                "The tool, the command, the output, a screenshot, and the date",
                "The severity, the CVE, the vendor, the version, and the patch",
                "What it is, where it is, its impact, proof, and the fix",
                "The who, the what, the when, the where, and the why"
              ],
              "answer": 2,
              "exp": "Flo: what it is (plain language), where (the specific system), impact (business terms), proof (reproducible, not a how-to for a criminal), and the fix (specific and prioritised — the part they'll read)."
            },
            {
              "q": "Why is the reporting phase the part beginners most often skip, and why is that a mistake?",
              "options": [
                "It's less exciting than exploitation",
                "It is genuinely optional on the majority of real engagements",
                "Automated tools already generate the whole report for you",
                "Clients rarely read the report, so its quality doesn't matter"
              ],
              "answer": 0,
              "exp": "Flo: the hack is the fun part; the report is the paid part. A modest finding with a clear, actionable write-up beats a spectacular one nobody can act on."
            },
            {
              "q": "A blue teamer studies offensive tooling primarily so they can do what?",
              "options": [
                "Run the same offensive tools directly against production systems",
                "Replace the detection products they already have deployed",
                "Satisfy the requirements of an offensive-security certification",
                "Recognise those tools in their logs — the scan, the crack, the C2 beacon"
              ],
              "answer": 3,
              "exp": "Flo: you can't recognise what you've never seen described. Knowing how red tooling behaves on the wire and in logs is what makes detection possible."
            },
            {
              "q": "Why do offence and defence belong in the same training track?",
              "options": [
                "Combining them simply makes the overall course longer and fuller",
                "The whole point of the attack is to make the defence better",
                "They are unrelated skills that happen to be taught side by side",
                "Compliance frameworks require both to be covered in one place"
              ],
              "answer": 1,
              "exp": "Flo: every red technique has a blue control and detection. A report a blue team can act on Monday morning is the whole job — the offence was how you learned what to write."
            }
          ]
        }
      ]
          ...
          "questions": [
            {
              ...
              "options": [
                "A firewall that sits between two trusting Active Directory domains",
                "The server that runs the domain",
                "The workstation used day to day by the most senior administrator",
                "A dedicated server that backs up user files across the domain"
              ],
              "answer": 1,
              "exp": "Bo: the DC runs the domain — it holds the database, authenticates users, grants access. Own a DC and you effectively own the domain, which is why they're the most protected machines in any network."
            },
            {
              "q": "What is stored in the NTDS.dit file on a Domain Controller?",
              "options": [
                "The Group Policy objects applied across the organisation",
                "The Windows security event logs for the controller",
                "The DNS records that the domain relies on to resolve names",
                "Every account and every password hash in the entire domain"
              ],
              "answer": 3,
              "exp": "Flo: NTDS.dit is the crown jewel — every account and password hash in the domain. If an attacker reads it, the whole domain is compromised. Protecting the DCs that hold it is most of defending AD."
            },
            {
              "q": "In Active Directory, what is the real security boundary?",
              "options": [
                "The forest, since domains within it trust one another",
                "The individual domain, which stands on its own",
                "The organisational unit that groups related objects",
                "The subnet that a given set of machines sits on"
              ],
              "answer": 0,
              "exp": "Flo: the forest, not the domain, is the security boundary. Domains within a forest trust each other, so a compromise in one can often reach the others — a common surprise for people who assume the domain is the wall."
            },
            {
              "q": "Nearly every Active Directory attack ultimately aims to compromise which group?",
              "options": [
                "Remote Desktop Users, who can log into machines interactively",
                "Backup Operators, who can read and restore protected files",
                "Domain Admins, the group that effectively controls everything",
                "Authenticated Users, which every domain member belongs to"
              ],
              "answer": 2,
              "exp": "Bo: Domain Admins control everything. Attackers don't try to 'hack the network' — they try to become Domain Admin, because that's effective ownership of every machine in the domain."
            },
            {
              "q": "Why are service accounts a favourite target in AD attacks?",
              "options": [
                "They always have multi-factor authentication enforced on them",
                "They cannot be enumerated by an ordinary authenticated user",
                "They're often over-privileged, rarely have MFA",
                "They only ever run on the Domain Controllers themselves"
              ],
              "answer": 2,
              "exp": "Bo: service accounts run software, not people. They tend to be over-privileged, exempt from MFA, and carry static weak passwords — the soft underbelly of the domain."
            },
            {
              ...
              "options": [
                ...
                "A password hash that other services can validate directly",
                "An NTLM challenge-and-response exchanged with the server",
                "A browser session cookie scoped to the internal network"
              ],
              "answer": 0,
              "exp": "Bo: Kerberos is ticket-based — log in once, get a TGT, then use it to request tickets for specific services. Almost every AD attack is a clever abuse of how those tickets are issued, stored, or trusted."
            },
            {
              "q": "Pass-the-hash attacks are possible because of which older authentication protocol?",
              "options": [
                "Kerberos, which issues time-limited service tickets",
                "LDAP, which the directory uses for queries and lookups",
                "RADIUS, which centralises remote-access authentication",
                "NTLM, which authenticates using the password hash itself"
              ],
              "answer": 3,
              "exp": "Bo: NTLM authenticates with the password HASH, so an attacker never needs the plaintext — a stolen hash is enough to log in elsewhere. It survives for compatibility, and it's why pass-the-hash works."
            },
            {
              "q": "An attacker gains control of a Domain Controller. What's the most accurate assessment?",
              "options": [
                "They can read the files on that single server and nothing more",
                "They effectively control the entire domain, not just that one server",
                "They still need each individual user's plaintext password",
                "The impact is limited strictly to the compromised controller"
              ],
              "answer": 1,
              "exp": "Bo: the DC is the domain. Control it and you can read every hash, forge trust, and reach every joined machine — which is why 'protect the DCs and who can log into them' is the core of AD defence."
            },
            {
              "q": "In Active Directory Users and Computers, what is an Organizational Unit (OU) — like the 'Sales' folder containing East, West and Pacific?",
              "options": [
                "A security group that grants its members access to resources",
                "A container an admin creates to organize objects, apply Group Policy, and delegate control",
                "A built-in system folder that cannot be changed",
                "Another name for the domain itself"
              ],
              "answer": 1,
              "exp": "Flo: an OU is an admin-created folder for organizing users/computers, targeting Group Policy, and delegating who can manage them. It is NOT a security group — groups grant access, OUs organize and govern. The Sales → East/West/Pacific tree is an org chart rendered in AD."
            },
            {
              "q": "You want to link a Group Policy Object to accounts sitting in the default 'Users' folder in ADUC, but you can't. Why?",
              "options": [
                "Users is a container, not an OU — GPOs link to OUs (and the domain/site), not to default containers",
                "Group Policy has been disabled on the domain",
                "You need Enterprise Admin rights to link any GPO",
                "The Users folder is empty so there is nothing to link to"
              ],
              "answer": 0,
              "exp": "Flo: 'Users' and 'Computers' are default containers, not OUs, and you cannot link a GPO to a container. That single difference is why real orgs move accounts out of Users into OUs — so policy and delegation can actually be applied. Classic exam trap."
            }
          ]
        },
        {
          ...
          "questions": [
            {
              "q": "After gaining one low-privilege domain user, what's the attacker's smartest FIRST move?",
              "options": [
                "Immediately attempt to escalate straight to Domain Admin",
                "Map the domain — users, groups, computers, service accounts",
                "Delete the security logs to cover the initial intrusion",
                "Deploy ransomware across the machines already reachable"
              ],
              "answer": 1,
              "exp": "Bo: AD is generous with information to any authenticated user. Mapping before attacking reveals the paths — attacking blind just makes noise and misses the route sitting right there."
            },
            {
              "q": "Modern AD offence is described as finding a PATH rather than a vulnerability. What does that mean?",
              "options": [
                "Locating a single unpatched CVE on an internet-facing host",
                "Scanning the domain for machines with open network ports",
                "Cracking the built-in Administrator account's password directly",
                "Chaining permissions and access"
              ],
              "answer": 3,
              "exp": "Bo: attack-path mapping graphs 'who can reach Domain Admin, and how.' A secure-looking domain often has a three-hop chain of legitimate permissions leading to full compromise. You find the path, not a bug."
            },
            {
              "q": "Which piece of AD recon is the 'hidden map' where many modern attacks live?",
              "options": [
                "Permissions and ACLs — who can do what to whom",
                "The list of open network ports on each joined machine",
                "The DNS records the domain uses to resolve names",
                "The current patch level of each server in the domain"
              ],
              "answer": 0,
              "exp": "Bo: ACLs are the quiet map. If a low-priv account can reset a privileged account's password or add itself to a group, that's a domination path with no exploit — it only shows up when you graph permissions."
            },
            {
              "q": "Why should DEFENDERS run the same attack-path mapping tools that attackers use?",
              "options": [
                "It is a formal requirement of most compliance frameworks",
                "It generates additional alerts that improve SOC coverage metrics",
                "The attacker's map and the defender's map are identical",
                "It is the only supported way to launch attacks on other companies"
              ],
              "answer": 2,
              "exp": "Flo: if you can see the three-hop path to Domain Admin before the attacker does, you can cut one link and close it. Same tool, same graph — used to harden instead of exploit."
            },
            {
              "q": "How much domain information can a typical authenticated low-privilege user read?",
              "options": [
                "Almost nothing beyond the details of their own account",
                "Only the specific resources an admin has explicitly shared",
                "A large amount — users, groups, computers",
                "Only their own account, and nothing about other objects"
              ],
              "answer": 2,
              "exp": "Bo: AD is designed to be queryable, so any authenticated user can enumerate huge amounts of structure. That's why a single low-priv foothold is so valuable — it unlocks the whole map."
            },
            {
              "q": "What does enumerating the domain TRUSTS help an attacker understand?",
              "options": [
                "How this domain relates to others and whether a path crosses between them",
                "Which individual users across the domain personally trust one another",
                "The password-complexity policy currently enforced across all accounts",
                "The schedule on which backups of the domain controllers are taken"
              ],
              "answer": 0,
              "exp": "Bo: trusts define relationships between domains and forests. Since the forest is the real boundary, mapping trusts shows whether compromising one domain opens a route into another."
            },
            {
              "q": "A graph shows your low-priv user can reset a help-desk account's password, and that account is local admin on a server where a Domain Admin is logged in. What is this?",
              "options": [
                "A false positive the mapping tool should not have reported",
                "A normal, safe configuration that needs no remediation",
                "An NTLM relay attack against the help-desk workstation",
                "A viable privilege-escalation path leading to Domain Admin"
              ],
              "answer": 3,
              "exp": "Bo: a textbook chain — password reset, admin on the box, steal the Domain Admin's session. Three legitimate permissions combine into total compromise. Break any single link and the path dies."
            },
            {
              "q": "What's the defender's takeaway from attack-path mapping their own domain?",
              "options": [
                "Buy additional endpoint-protection tooling for every host",
                "Identify and break the shortest chains to privileged accounts",
                "Disable all service accounts across the whole environment",
                "Rotate the KRBTGT account's key on a daily schedule"
              ],
              "answer": 1,
              "exp": "Flo: the goal is fewer and longer paths to Domain Admin. Removing dangerous ACLs, fixing over-privileged accounts, and controlling where admins log in all cut links out of the graph."
            }
          ]
        },
        {
          ...
          "questions": [
            {
              "q": "Kerberoasting works because of which design feature?",
              "options": [
                "The Domain Controller keeps every account password stored in plaintext on disk",
                "Any user can request a service ticket that's encrypted with the service account's hash",
                "NTLM has been disabled everywhere, forcing all authentication through Kerberos",
                "Multi-factor authentication is automatically bypassed when a service ticket issues"
              ],
              "answer": 1,
              "exp": "Bo: it abuses a feature — any user can request a service ticket, encrypted with the service account's hash. Take it offline, crack the often-weak password, and you have real credentials, quietly."
            },
            {
              "q": "What makes Kerberoasting especially hard to notice?",
              "options": [
                "It briefly crashes the Domain Controller, and the outage masks the activity",
                "It requires the attacker to already hold Domain Admin rights before starting",
                "It always triggers a multi-factor authentication prompt on the targeted account",
                "The request looks normal; the cracking happens offline on the attacker's hardware"
              ],
              "answer": 3,
              "exp": "Bo: requesting a service ticket is completely normal behaviour. The attack — cracking the hash — happens offline where nobody's watching. That's why strong service-account passwords matter so much."
            },
            {
              "q": "AS-REP roasting targets accounts that have which setting?",
              "options": [
                "Kerberos pre-authentication disabled on the account",
                "Multi-factor authentication enabled for the account",
                "A weak account-lockout policy applied to the account",
                "Unconstrained delegation enabled for the account"
              ],
              "answer": 0,
              "exp": "Bo: accounts with pre-auth off hand out a crackable blob to anyone who asks — no foothold even required. Re-enabling pre-authentication is the direct fix."
            },
            {
              "q": "In a pass-the-hash attack, what does the attacker use to authenticate?",
              "options": [
                "The account's recovered plaintext password",
                "A forged Kerberos golden ticket for the account",
                "The password hash itself, replayed through NTLM",
                "A physical smart card issued to the account"
              ],
              "answer": 2,
              "exp": "Bo: NTLM accepts the hash in place of the plaintext, so a stolen hash logs the attacker in as that user on other systems — no cracking required. Modern credential protections and reducing NTLM shut it down."
            },
            {
              "q": "What's the single best defence against Kerberoasting?",
              "options": [
                "Disabling Kerberos authentication across the entire domain environment",
                "Blocking every service-ticket request from ordinary users at the Domain Controller",
                "Long random or managed service-account passwords that rotate automatically",
                "Re-enabling AS-REP pre-authentication responses for every account in the domain"
              ],
              "answer": 2,
              "exp": "Flo: if the service-account password is long and random — or auto-rotated by a managed service account — the offline crack fails in any realistic time. That directly defeats the attack."
            },
            {
              "q": "Pass-the-ticket differs from pass-the-hash in that the attacker replays what?",
              "options": [
                "A Kerberos ticket stolen from a machine's memory",
                "A password hash captured from the SAM database",
                "A cracked plaintext password recovered offline",
                "A Group Policy object pushed from the controller"
              ],
              "answer": 0,
              "exp": "Bo: pass-the-ticket steals an actual Kerberos ticket from memory and replays it — becoming whoever's ticket it was, with no password at all. Keeping privileged tickets off reachable machines is the counter."
            },
            {
              "q": "Why are service accounts the 'soft underbelly' for these attacks?",
              "options": [
                "They are automatically enrolled in multi-factor authentication when created",
                "They cannot be enumerated at all by a standard authenticated domain user",
                "They are always placed in the Domain Admins group automatically by default",
                "Weak passwords that rarely change plus broad privileges make them ideal targets"
              ],
              "answer": 3,
              "exp": "Bo: a weak, static service-account password is exactly what makes an offline crack succeed, and the account's broad rights make the payoff huge. Fix service accounts first in most domains."
            },
            {
              "q": "A domain has many service accounts with weak passwords. As a defender you should:",
              "options": [
                "Enable AS-REP roasting on them so you can test their strength",
                "Prioritise rotating them to long random or managed passwords first",
                "Delete every service account across the whole environment",
                "Ignore it as long as MFA is enabled for the human users"
              ],
              "answer": 1,
              "exp": "Flo: weak service-account passwords are the highest-likelihood Kerberoasting win in the domain. Understanding the attack is exactly what tells you this is the first thing to fix, not the tenth."
            }
          ]
        },
        {
          ...
          "questions": [
            {
              ...
              "options": [
                "The scheduled refresh of Group Policy that runs on every domain member",
                "Domain replication — asking a DC for password data as if you were another DC",
                "The DNS zone-transfer mechanism running between the domain name servers",
                "The certificate-enrollment process that domain machines use to request certs"
              ],
              "answer": 1,
              "exp": "Bo: with the right replication rights, an attacker asks a DC to hand over hashes 'as if' they were a peer DC — pulling any account's hash without ever logging into the DC. Tightly control who holds replication rights."
            },
            {
              "q": "A golden ticket becomes possible when an attacker steals which account's key?",
              "options": [
                "The built-in Administrator account for the domain",
                "A single Domain Admin's ordinary user password",
                "The service account that runs the domain's DNS role",
                "The KRBTGT account, whose key signs every Kerberos ticket"
              ],
              "answer": 3,
              "exp": "Bo: the KRBTGT hash signs every Kerberos ticket. Steal it and you can forge tickets for anyone, with any privileges, that the domain will trust — the closest thing AD has to a skeleton key."
            },
            {
              "q": "Why are golden-ticket compromises so hard to fully recover from?",
              "options": [
                "Resetting user passwords doesn't help",
                "The Domain Controller must be rebuilt entirely from scratch",
                "Kerberos has to be disabled across the domain permanently",
                "Every user in the domain must re-enrol in multi-factor auth"
              ],
              "answer": 0,
              "exp": "Bo: the forged tickets are signed by the master key, so only rotating KRBTGT — twice, correctly — evicts the attacker. Password resets alone don't touch it, which is why AD incidents are so brutal."
            },
            {
              "q": "A silver ticket differs from a golden ticket in that it is:",
              "options": [
                "More powerful, granting control over the entire forest at once",
                "Usable only by accounts already in the Domain Admins group",
                "Narrower — a forged ticket for one specific service, quieter and targeted",
                "Signed by the domain's DNS server rather than the KRBTGT key"
              ],
              "answer": 2,
              "exp": "Bo: a silver ticket forges access to a single service rather than the whole domain. It's stealthier and more surgical — useful when an attacker wants to stay quiet on one target."
            },
            {
              "q": "ACL abuse can lead to domain domination with what?",
              "options": [
                "A previously unknown zero-day in the Domain Controller",
                "A cracked copy of the domain's KRBTGT account key",
                "No exploit at all — just misconfigured permissions an attacker chains",
                "A distributed denial-of-service attack on the controllers"
              ],
              "answer": 2,
              "exp": "Bo: ACL abuse is pure misconfiguration. If permissions let a weak account reset a strong account's password or add itself to a group, that's a path to domination with nothing to 'exploit' — so audit ACLs."
            },
            {
              "q": "Kerberos delegation, when misconfigured, allows an attacker to do what?",
              "options": [
                "Impersonate high-privilege users to reach systems they shouldn't",
                "Disable Kerberos authentication for the whole domain",
                "Read the NTDS.dit database directly off the controller",
                "Reset the KRBTGT account's key without any privileges"
              ],
              "answer": 0,
              "exp": "Bo: delegation lets a service act on a user's behalf. Misconfigured, it becomes an impersonation primitive — a well-known escalation path defenders must specifically hunt for and lock down."
            },
            {
              "q": "Most domination techniques share which prerequisite that defenders can attack?",
              "options": [
                "An open port exposed directly to the public internet",
                "A firewall that has been disabled on the Domain Controller",
                "An unpatched web server running somewhere in the domain",
                "A highly privileged credential or dangerous permission obtained first"
              ],
              "answer": 3,
              "exp": "Flo: DCSync, golden tickets, delegation abuse — nearly all need a privileged credential or dangerous right first. Deny that with tiering, least privilege and clean ACLs and most of the endgame is unreachable."
            },
            {
              "q": "An attacker has obtained the KRBTGT hash. What's the correct eviction step?",
              "options": [
                "Reset the password of the single compromised user account",
                "Rotate the KRBTGT account's password twice",
                "Reboot all of the domain's controllers simultaneously",
                "Disable NTLM authentication across the whole domain"
              ],
              "answer": 1,
              "exp": "Bo: only a double KRBTGT rotation invalidates the forged golden tickets. It's a delicate procedure most orgs have never practised — which is exactly why knowing it sets you apart on an IR team."
            }
          ]
        },
        {
          ...
          "questions": [
            {
              "q": "What's the single highest-value control for defending Active Directory?",
              "options": [
                "Antivirus software installed and updated on every host",
                "Tiered administration",
                "A stronger firewall ruleset at the network perimeter",
                "Taking full backups of the environment every single day"
              ],
              "answer": 1,
              "exp": "Flo: nearly every catastrophic AD breach ends with a Domain Admin credential on a machine the attacker reached. If the powerful ticket is never on that machine, it can't be stolen — tiering removes the most common path."
            },
            {
              "q": "Randomising the LOCAL administrator password on every machine primarily stops what?",
              "options": [
                "Kerberoasting of the domain's service accounts",
                "Golden-ticket forgery using the KRBTGT key",
                "AS-REP roasting of pre-auth-disabled accounts",
                "One cracked local-admin password unlocking every machine"
              ],
              "answer": 3,
              "exp": "Flo: when every machine shares a local-admin password, one crack owns them all. Randomising per machine, with automatic management, kills that lateral-movement shortcut."
            },
            {
              ...
              "options": [
                ...
                "A user successfully logging into a workstation during normal business hours",
                "A single failed password attempt against one standard user account",
                "A new network file share suddenly being created on an ordinary member server"
              ],
              "answer": 0,
              "exp": "Flo: only DCs should replicate directory data. Replication requested by a non-DC is a strong DCSync indicator — a specific, monitorable tell that comes straight from understanding the attack."
            },
            {
              "q": "A sudden flood of service-ticket requests from one user is a possible sign of what?",
              "options": [
                "A golden-ticket forgery already in progress",
                "A pass-the-hash attack replaying NTLM hashes",
                "Kerberoasting, harvesting tickets to crack offline",
                "An ACL misconfiguration being exploited quietly"
              ],
              "answer": 2,
              "exp": "Flo: Kerberoasting means requesting service tickets to crack offline. An unusual burst from a single account is the behavioural tell — you can't detect it if you don't know the attack."
            },
            {
              "q": "Managed service accounts with automatic password rotation primarily defend against what?",
              "options": [
                "Golden-ticket forgery signed with the KRBTGT key",
                "DCSync replication pulls from a peer controller",
                "Kerberoasting, by making the offline crack infeasible",
                "NTLM relay attacks against member servers"
              ],
              "answer": 2,
              "exp": "Flo: automatic long random rotation makes the offline crack central to Kerberoasting fail. It's the cleanest fix for the service-account weakness the attack depends on."
            },
            {
              "q": "Why does studying AD ATTACKS make someone a better AD defender?",
              "options": [
                "Knowing what the attacker reaches for first tells you which controls to prioritise",
                "It does not help at all, because defence is an entirely separate discipline",
                "It is useful only for practitioners who work exclusively on offensive red teams",
                "Attacks and defences in Active Directory are fundamentally unrelated disciplines"
              ],
              "answer": 0,
              "exp": "Bo: red-informs-blue is strongest in AD. Because you've seen the attacker go straight for weak service accounts and exposed Domain Admin tickets, you know to fix those first instead of working a generic checklist."
            },
            {
              "q": "Least privilege and clean ACLs defend AD primarily by doing what?",
              "options": [
                "Speeding up the interactive login process for ordinary domain users",
                "Encrypting the NTDS.dit database file at rest on the domain controllers",
                "Disabling Kerberos in favour of a newer and stronger authentication protocol",
                "Removing the permission-based paths attackers chain toward Domain Admin"
              ],
              "answer": 3,
              "exp": "Bo: most escalation paths are chains of excessive permissions. Trimming rights and fixing dangerous ACLs deletes links from the attack graph, so fewer and longer routes to domination remain."
            },
            {
              "q": "You find a Domain Admin regularly logs into ordinary user workstations. What's the right guidance?",
              "options": [
                "It's fine as long as those workstations run antivirus",
                "Stop it — restrict admin logons to hardened admin systems only",
                "It's acceptable if the workstations have plenty of RAM",
                "Enable AS-REP on those accounts to compensate for it"
              ],
              "answer": 1,
              "exp": "Flo: a Domain Admin ticket on a regular workstation is exactly what pass-the-ticket and credential theft prey on. Restricting where privileged accounts log in — tiering — removes the single most exploited AD weakness."
            }
          ]
        }
      ]
...
export const LESSONS: Record<string, string> = {
  ...
  "ad1": "<h3>📖 WHAT THIS MODULE IS ABOUT</h3><p>The ground truth of Active Directory — the model every attack and defense depends on. Most people who 'know AD' can add a user and nothing more. This is the mental model that makes the rest click.</p><h3>🔑 KEY CONCEPTS IN BO &amp; FLO TERMS</h3><ul><li><strong>Domain</strong> (Bo) — users, computers and resources under one roof, sharing one identity database.</li><li><strong>Domain Controller (DC)</strong> (Bo) — the server that runs the domain. Own it, own everything.</li><li><strong>Forest</strong> (Flo) — one or more trusting domains. The forest, not the domain, is the real security boundary.</li><li><strong>NTDS.dit</strong> (Flo) — the DC's database of every account and password hash. The crown jewel. Read it and it's over.</li><li><strong>Kerberos</strong> (Bo) — ticket-based auth. Log in once, get a TGT, reuse it for services. Source of most AD attacks.</li><li><strong>NTLM</strong> (Bo) — the older, weaker protocol behind pass-the-hash. Authenticates with the hash, not the plaintext.</li><li><strong>Service accounts</strong> (Bo) — accounts that run software. Over-privileged, no MFA, weak static passwords. The favorite target.</li><li><strong>Domain Admin</strong> (Bo) — controls everything; the goal of nearly every attack.</li></ul><h3>🗂️ READING THE AD MAP — ADUC KEY CONCEPTS</h3><p>Open <strong>Active Directory Users and Computers</strong> (ADUC) — the console admins actually live in — and the whole domain shows up as a tree. Learn to read it, because that tree IS the attack map. (New to this? The diagram above labels every piece.)</p><ul><li><strong>The domain root</strong> — the very top (e.g. <code>Fabrikam.com</code> or your <code>lab.local</code>). Everything below it is the <em>domain partition</em>: every user, computer, group and OU in the domain.</li><li><strong>Default containers</strong> (Bo) — the folders AD ships with: <code>Builtin</code>, <code>Computers</code>, <code>Domain Controllers</code>, <code>Users</code>, plus <code>ForeignSecurityPrincipals</code>, <code>LostAndFound</code>, <code>System</code>. New machines land in <code>Computers</code>, new users in <code>Users</code> — until an admin organizes them.</li><li><strong>OU = Organizational Unit</strong> (Flo) — a folder an admin <em>creates</em> to organize objects, apply Group Policy to them, and <em>delegate</em> who can manage them. In the classic example <code>Sales</code> is an OU with sub-OUs <code>East</code>, <code>West</code>, <code>Pacific</code>, <code>Europe</code>, <code>Management</code>. This is the org chart, rendered.</li><li><strong>Container vs OU</strong> (Flo) — they look alike but aren't: you can link a GPO to an <em>OU</em>, not to a default <em>container</em> like <code>Users</code>. Exam bait, and it's why real orgs move accounts out of <code>Users</code> into OUs.</li><li><strong>Objects</strong> (Bo) — inside the tree: <strong>users</strong> (Leo, Virgo, Aquarius), <strong>computers</strong>, <strong>security groups</strong> (&lsquo;All Sales Personnel&rsquo;, &lsquo;Sales East/West&rsquo; — often <em>Global</em> groups that grant access), and even shared folders. Type + Description columns tell you what each one is.</li><li><strong>Why the attacker cares</strong> (Flo) — the OU layout leaks the org structure, group membership leaks who has privilege, and <em>delegated rights on an OU</em> are a classic quiet escalation path. Reading ADUC well is step one of every AD attack — and every AD defense.</li></ul><h3>🎯 COMMON PITFALLS</h3><ul><li>Thinking the domain is the security boundary. The forest is.</li><li>Not knowing what NTDS.dit is — every 'how'd they own everything?' story ends there.</li><li>Treating service accounts like normal accounts. They're the soft underbelly.</li><li>Trying to 'hack the network' instead of aiming at Domain Admin.</li></ul><h3>📝 BO CHEAT SHEET</h3><ul><li>DC runs the domain · own it = own everything</li><li>NTDS.dit = all accounts + all hashes = crown jewel</li><li>Forest = the real security boundary</li><li>Kerberos = tickets (TGT then service tickets) · NTLM = hash-based, weaker</li><li>Service accounts = over-privileged, no MFA, weak passwords</li><li>The goal is always Domain Admin</li></ul><h3>💼 REAL WORLD</h3><p>An attacker phishes one employee and lands as a nobody user. Useless? No — because AD is one giant shared identity system, that foothold lets them read the whole map, hunt weak service accounts, and start chaining toward Domain Admin. Understanding that AD is ONE connected kingdom, not a pile of separate machines, is what turns 'I got a low-priv user' into 'I own the company.' Same understanding tells the defender why that one phished user matters so much.</p>",
