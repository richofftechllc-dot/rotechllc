...
const ACCENT: Record<string, { border: string; chip: string }> = {
  green: { border: "border-green-500/20", chip: "bg-green-500/20" },
  yellow: { border: "border-yellow-500/20", chip: "bg-yellow-500/20" },
  blue: { border: "border-blue-500/20", chip: "bg-blue-500/20" },
};
  ...
  return (
    <>
      <section className="max-w-6xl mx-auto px-6 pt-16 pb-4 text-center">
        <div className="text-orange-500 font-bold tracking-widest text-sm mb-4">HELP &amp; SUPPORT</div>
      ...
      <section id="agents" className="bg-gradient-to-br from-green-950/30 to-black py-24 border-t border-white/5 scroll-mt-20">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {AGENTS.map((a) => (
              <div key={a.id} className={`bg-zinc-900 border ${ACCENT[a.accent].border} rounded-2xl p-5`}>
      ...
      <section className="bg-zinc-950 py-24 border-t border-white/5">
        <div className="max-w-5xl mx-auto px-6">
          <div className="text-center mb-12">
            <div className="text-orange-500 font-bold tracking-widest text-sm mb-4">READ IT YOURSELF</div>
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            {RESOURCES.map((r) => (
              <a
                key={r.href}
                href={r.href}
                className="block bg-zinc-900 border border-white/10 rounded-2xl p-6 hover:border-orange-500/50 transition"
              >
      ...
      <section className="bg-gradient-to-br from-orange-500/10 to-red-500/10 py-24 border-t border-white/5">
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            ...
            <a href="/book" className="inline-block px-8 py-4 border border-white/20 rounded-lg text-lg hover:bg-white/5">
              Book a 1-on-1
            </a>
          </div>
        </div>
