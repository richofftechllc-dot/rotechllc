...
export default function Login() {
  ...
  return (
    <main className="min-h-[80vh] flex items-center justify-center px-6 py-16">
      <div className="bg-rot-surface border border-rot-line rounded-2xl p-10 max-w-md w-full">
        ...
        <p className="text-rot-muted text-center text-sm mb-8">Use Discord or your access code</p>

        <a href="/api/auth/discord"
          className="flex items-center justify-center gap-3 w-full py-3 rounded-lg mb-4 font-bold text-rot-fg transition-colors"
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="flex-1 h-px bg-rot-sunken"></div>
          <span className="text-rot-faint text-xs">or use access code</span>
          <div className="flex-1 h-px bg-rot-sunken"></div>
        </div>

        <label className="block text-xs uppercase tracking-wider text-rot-muted mb-2">Access Code</label>
        ...
        {err && <div className="text-red-600 text-sm text-center mt-3">{err}</div>}
        <div className="text-center text-xs text-rot-faint mt-6">
          ...
          Not a member? <a href="/membership" className="text-rot-accent">Join ROT — ${PRICING.yearly}/year</a>
        </div>
      </div>
    </main>
  );
