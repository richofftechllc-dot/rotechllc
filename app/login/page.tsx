...
export default function Login() {
  ...
  async function signIn() {
    ...
    try {
      ...
      window.location.href = params.get("from") || "/quiz";
    } catch (e) {
  ...
  return (
    <main className="min-h-[80vh] flex items-center justify-center px-6 py-16">
      <div className="bg-zinc-900 border border-white/10 rounded-2xl p-10 max-w-md w-full">
        ...
        <input value={code} onChange={e => setCode(e.target.value)} onKeyDown={e => e.key === "Enter" && !loading && signIn()}
        ...
        <button onClick={signIn} disabled={loading} className="w-full py-3 bg-gradient-to-r from-orange-500 to-red-500 text-white font-bold rounded-lg uppercase tracking-wider disabled:opacity-50">
