...
export default async function Calls() {
  ...
  return (
    <main className="max-w-5xl mx-auto px-6 py-16">
      ...
      <p className="text-rot-muted text-center mb-12 max-w-2xl mx-auto text-lg">
      ...
      <div className="border border-rot-accent/30 rounded-2xl p-6 md:p-10 max-w-4xl mx-auto bg-gradient-to-br from-orange-500/5 to-red-500/5">
        ...
        {ev ? (
          <>
            ...
            {when && <p className="text-rot-muted text-center mb-6">{when.date} · {when.time}</p>}
            {ev.description && (
              <p className="text-rot-muted text-center max-w-2xl mx-auto mb-8 leading-relaxed">{ev.description}</p>
            )}
            <div className="flex flex-col items-center gap-3">
              ...
              <a href={DISCORD_LINK} target="_blank" rel="noopener noreferrer" className="px-7 py-3.5 border border-rot-line-strong text-rot-fg font-bold rounded-lg hover:bg-rot-sunken text-base">
        ...
        <div className="text-center text-rot-faint text-xs mt-8">
          Sessions run in Discord voice + screen-share. Recordings posted in the same server.
        </div>
      </div>

      {/* JULY COHORTS — waitlist */}
      <div className="border border-rot-accent/30 rounded-2xl p-6 md:p-10 max-w-4xl mx-auto bg-rot-sunken mt-10">
        <div className="text-center mb-6">
          <div className="text-rot-accent text-xs font-bold tracking-widest mb-2">DROPPING THIS MONTH</div>
          <h2 className="text-3xl md:text-4xl font-black mb-3">July Cohorts are coming. 🚀</h2>
          <p className="text-rot-muted max-w-2xl mx-auto">
            Live, instructor-led cert cohorts kick off when Bo&apos;s back. Seats are limited. Get on the waitlist now and we&apos;ll <b className="text-rot-fg">text you the second they drop</b> — first dibs before it goes public.
          </p>
        </div>
      ...
      <div className="border border-rot-line rounded-2xl p-6 md:p-8 max-w-4xl mx-auto bg-rot-sunken mt-10">
        <div className="text-rot-muted text-xs font-bold tracking-widest text-center mb-3 uppercase">From the Last Session</div>
        <h3 className="text-xl md:text-2xl font-bold text-center mb-4">January 14, 2026 — Community Call</h3>
        <div className="aspect-video rounded-xl overflow-hidden bg-rot-bg border border-rot-line">
          <iframe
        ...
        <p className="text-rot-faint text-xs text-center mt-4">Past sessions live on YouTube. Subscribe to <a href="https://www.youtube.com/@RichOffTechLLc" target="_blank" rel="noopener noreferrer" className="text-rot-accent hover:text-rot-accent font-bold">@RichOffTechLLc</a> for the full archive.</p>
      </div>
    </main>
  );
}
