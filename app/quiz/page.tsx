...
export default function Quiz() {
  ...
  // Free fallback so Bo is never silent: if ElevenLabs is unavailable (free plan), use the browser voice.
  function speakBrowser(text: string, idx: number, gen: number) {
    if (typeof window === "undefined" || !("speechSynthesis" in window)) { setSpeakingIdx(null); return; }
    if (gen !== speakGenRef.current) return;
    window.speechSynthesis.cancel();
    const u = new SpeechSynthesisUtterance(text.replace(/[*_#`>]/g, "").slice(0, 600));
    u.rate = 1.05; setSpeakingIdx(idx);
  ...
  async function speak(text: string, idx: number, p: "bo" | "flo" = persona) {
    ...
    try {
      ...
      if (gen !== speakGenRef.current) return;
      if (!r.ok) { speakBrowser(text, idx, gen); return; }
    ...
    } catch { speakBrowser(text, idx, gen); }
  }
