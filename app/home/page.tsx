...
export default function MemberHome() {
  ...
  if (loading) {
    return <main className="min-h-screen bg-rot-bg text-rot-fg grid place-items-center"><p className="text-rot-fg/50">Loading your home…</p></main>;
  }
  if (!d) {
    return (
      <main className="min-h-screen bg-rot-bg text-rot-fg grid place-items-center px-6 text-center">
        <div>
          <p className="text-rot-fg/60 mb-3">We couldn&apos;t load your home right now.</p>
          <Link href="/login" className="text-rot-accent underline">Log in again</Link>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-rot-bg text-rot-fg">
      <div className="max-w-5xl mx-auto px-5 py-10">
        {/* Greeting + status */}
        <div className="flex items-center justify-between flex-wrap gap-3 mb-8">
          <div>
            <p className="text-rot-accent text-xs font-bold tracking-[0.2em] uppercase mb-1">Your Home</p>
            <h1 className="text-3xl sm:text-4xl font-extrabold">Yo, {d.name.split(" ")[0]} 👋</h1>
          </div>
          <div className="flex items-center gap-2">
            {d.isCoach && <Link href="/admin" className="text-xs px-3 py-1.5 rounded-full bg-rot-fg text-white font-semibold hover:bg-white/90 transition">🛠 Coach CRM →</Link>}
            {d.track && <span className="text-xs px-3 py-1.5 rounded-full border border-rot-line text-rot-fg/70">{d.track}</span>}
        ...
        <Link href="/quiz" className="block rounded-2xl border border-rot-accent/30 bg-gradient-to-br from-orange-500/10 to-transparent p-6 mb-5 hover:border-orange-500/60 transition">
          ...
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className="text-rot-fg/50 text-sm mt-1">
        ...
        <div className="grid md:grid-cols-2 gap-5 mb-5">
          <div className="rounded-2xl border border-rot-line bg-rot-sunken p-6">
            <p className="text-xs uppercase tracking-wider text-rot-fg/40 font-bold mb-3">Exam readiness</p>
            <div className="flex items-baseline gap-2">
              <span className="text-4xl font-extrabold text-rot-accent">{readiness !== null ? `${readiness}%` : "—"}</span>
          ...
          <div className="rounded-2xl border border-rot-line bg-rot-sunken p-6">
            <p className="text-xs uppercase tracking-wider text-rot-fg/40 font-bold mb-3">Upcoming 1-on-1s</p>
            {d.bookings.length > 0 ? (
              <div className="space-y-2">
                {d.bookings.slice(0, 3).map((b, i) => (
                  <div key={i} className="text-sm">
                    <span className="text-rot-fg font-semibold">{fmtSlot(b.slot)}</span>
                    <span className="text-rot-fg/50"> · {b.topic || b.label || "Session"}{b.coach ? ` w/ ${b.coach}` : ""}</span>
                  </div>
                ))}
              </div>
            ) : (
        ...
        {/* Quick actions */}
        <p className="text-xs uppercase tracking-wider text-rot-fg/40 font-bold mb-3">Jump back in</p>
        ...
        {/* Certs */}
        {d.certs.length > 0 && (
          <div className="mt-6 rounded-2xl border border-rot-line bg-rot-sunken p-5">
            <p className="text-xs uppercase tracking-wider text-rot-fg/40 font-bold mb-2">Your certs</p>
            <div className="flex flex-wrap gap-2">
              {d.certs.map((c, i) => (
                <span key={i} className="text-sm px-3 py-1 rounded-full bg-rot-accent/10 border border-rot-accent/30 text-rot-accent">{c}</span>
              ))}
            </div>
          </div>
        )}
...
function Action({ href, emoji, label, sub }: { href: string; emoji: string; label: string; sub: string }) {
  return (
    <Link href={href} className="rounded-xl border border-rot-line bg-rot-sunken p-4 hover:border-rot-accent/30 hover:bg-white/[0.07] transition">
      <div className="text-2xl mb-1">{emoji}</div>
      <div className="font-semibold text-sm">{label}</div>
      <div className="text-rot-fg/40 text-xs">{sub}</div>
    </Link>
  );
}
